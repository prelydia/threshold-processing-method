...
def calculate_distance(x1, y1, x2, y2):
    ...
    returns distance = √(x2 - x1)^2 + (y2 - y1)^2

    where (x1, y1) are centroids of the first particle,
          (x1, y1) are centroids of the second particle.
...
def calculate_radius(area):
    ...
    returns radius = √(area/π)

    where area is the area of particle.

    """
    return math.sqrt(area / math.pi)

def are_dimers(p1, p2):
    """
    The function to check whether two particles are dimmer. 
    
    A connected component is a dimmer if each of the two particles in 
    ...
    r1 = calculate_radius(area1)
    r2 = calculate_radius(area2)

    distance = calculate_distance(x1, y1, x2, y2)
    sum_radii = r1 + r2
    error = sum_radii / 10 # error

    # if the sum of the radii of the particles is less than the distance between them, then the particles are not dimers
...
def is_chain(graph, dict):
    ...
    A connected component is a chain if two particles on its 
...
def watershed_algorithm(image, type):
    ...
    coords = peak_local_max(dist, min_distance=5, labels=thresh)
...
default_type = cv2.THRESH_BINARY | cv2.THRESH_OTSU
...
threshold_area = get_threshold_area(labels)
...
for i in range(len(particle_centers)):
    for j in range(len(particle_centers)):
        ...
        if are_dimers(particle_centers[i], particle_centers[j]) and i != j:
